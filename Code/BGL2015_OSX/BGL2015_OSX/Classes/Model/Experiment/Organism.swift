...
final public class Organism {
    ...
    public init(identifier: Identifier) {
        ...
        // Note that sensors X and Y, and the corresponding sensory input
        // neurons are in the first area (i.e., areas[0]) of their respective
        // regions. 
        //
        // Sensor Sr is in the second area (i.e., areas[1]) area of the
        // sensor region. Its sensory input neuron is in the second area of
        // the sensory input region.
        //
        xSensor =
            brain.appendSensor(SimpleBinarySensor(network: brain, idString: "X"),
                               areaIndex: 0) as! BinarySensor
        ySensor =
            brain.appendSensor(SimpleBinarySensor(network: brain, idString: "Y"),
                               areaIndex: 0) as! BinarySensor
        srSensor =
            brain.appendSensor(SimpleBinarySensor(network: brain, idString: "Sr"),
                               areaIndex: 1) as! BinarySensor
