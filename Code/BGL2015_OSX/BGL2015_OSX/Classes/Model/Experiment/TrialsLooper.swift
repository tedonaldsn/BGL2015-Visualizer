...
final public class TrialsLooper {
    ...
    // Call the progress callback after each n trials
    var progressCallbackFrequency: Int = 10
    ...
    public var totalTrials: Int {
        return TrialsLooper.totalTrials
    }
    public var trialNumber: Int {
        return priv_trialNumber
    }
    
    public var isRunComplete: Bool {
        return priv_stepData.count == TrialsLooper.totalSteps
        && priv_finalChoiceSteps.count == TrialsLooper.totalChoiceTrials
    }
    ...
    public typealias ProgressCallback = (_: TrialsLooper) -> Void
    
    var progressCallback: ProgressCallback? = nil
    
    // MARK: Run
    
    public func runTrials(progressCallback: ProgressCallback? = nil) {
        ...
        priv_endOfSessionDataCollection()
        ...
        // priv_printMinMaxDiscrepancySignals()
    }
    
/*
    // MARK: Debug Code
    private var max_dopaminergicSignal: Double = 0.0
    private var min_dopaminergicSignal: Double = 0.0
    private var max_hippocampalSignal: Double = 0.0
    private var min_hippocampalSignal: Double = 0.0
    private func priv_recordMinMaxDiscrepancySignals() -> Void {
        
        let dopaminergicSignal = organism.brain.dopaminergicSignal
        let hippocampalSignal = organism.brain.hippocampalSignal
        
        if dopaminergicSignal < min_dopaminergicSignal {
            min_dopaminergicSignal = dopaminergicSignal
        } else if dopaminergicSignal > max_dopaminergicSignal {
            max_dopaminergicSignal = dopaminergicSignal
        }
        
        if hippocampalSignal < min_hippocampalSignal {
            min_hippocampalSignal = hippocampalSignal
        } else if hippocampalSignal > max_hippocampalSignal {
            max_hippocampalSignal = hippocampalSignal
        }
    }
    private func priv_printMinMaxDiscrepancySignals() -> Void {
        Swift.print("")
        Swift.print("Dopaminergic Signal: \(min_dopaminergicSignal)-\(max_dopaminergicSignal)")
        Swift.print("Hippocampal Signal: \(min_hippocampalSignal)-\(max_hippocampalSignal)")
        Swift.print("")
    }
*/
    ...
    public func saveSessionData(_ managedObjectContext: NSManagedObjectContext) throws -> Void {
        let sessionEntity =
            NSEntityDescription.entity(forEntityName: Session.entityName,
                                              in: managedObjectContext)
        let sessionRecord = Session(entity: sessionEntity!,
                                    insertInto: managedObjectContext)
        
        sessionRecord.recordStart(priv_sessionStartedAt)
        sessionRecord.recordStop()
        assert(sessionRecord.isValidInvariants)
        
        let stepEntity = NSEntityDescription.entity(forEntityName: Step.entityName,
                                                           in: managedObjectContext)
        
        
        for stepData in priv_stepData {
            let step = Step(entity: stepEntity!,
                            insertInto: managedObjectContext)
            
            step.session = sessionRecord
            
            stepData.copyTo(step)
            
            if stepData.trialNumber > TrialsLooper.totalTrainingTrials
                && stepData.trialStepNumber == TrialsLooper.maxStepsPerTrial {
                
                priv_finalChoiceSteps.append(stepData)
            }
        }
        
        sessionRecord.r1Count = Int64(priv_choiceSummaryData.r1Count)
        sessionRecord.r2Count = Int64(priv_choiceSummaryData.r2Count)
        sessionRecord.mPrime1Activation = priv_choiceSummaryData.mPrime1Activation
        sessionRecord.mPrime2Activation = priv_choiceSummaryData.mPrime2Activation
        sessionRecord.smPrimePrime1Weight = priv_choiceSummaryData.smPrimePrime1Weight
        sessionRecord.smPrimePrime2Weight = priv_choiceSummaryData.smPrimePrime2Weight
        
        // Note that this saves not just the data added within this method, but
        // unsaved changes in the same context.
        //
    ...
    fileprivate func priv_runTrial() {
        ...
        while priv_trialStepNumber <= TrialsLooper.nonSrTimeStepsPerTrial {
            ...
            // priv_recordMinMaxDiscrepancySignals()
        ...
        if priv_trialNumber % progressCallbackFrequency == 0 {
    ...
    public func priv_endOfStepDataCollection() -> Void {
        ...
        let neuralNet: BASelectionistNeuralNetwork.Network = organism.brain
        let stepStateData: Data? = NSKeyedArchiver.archivedData(withRootObject: neuralNet)
        stepData.networkState = stepStateData
    ...
    public func priv_endOfSessionDataCollection() -> Void {
        
        logger.logTrace("priv_endOfSessionDataCollection")
    ...
    } // end priv_endOfSessionDataCollection
